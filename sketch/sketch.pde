...
void view(ArrayList<Float> lis){
	float scl = width/2/lis.size();
	float maxD = dist(0, 0, width/2, height);
	for (int i = 0; i < lis.size(); i++){
		int colo = int(map(lis.get(i), 0, maxD, 220, 0));
		int size = int(map(lis.get(i), 0, maxD, height, 15));
		push();
		rectMode(CENTER);
		noStroke();
    	fill(colo);
		translate(i*scl+width/2, height/2);
		rect(scl/2, 0, scl, size);
		pop();
	}
}
